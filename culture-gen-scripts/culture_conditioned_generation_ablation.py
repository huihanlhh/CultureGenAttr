"""
This script configures the culture-conditioned generation ablation study.
"""
import os
import json
import argparse
...
def prompt_generator(role, nationality, gender, topic, chat=False, mode="continuation", probably=False):
    ...
    # Jailbreaking prompt for the instruct model and generate culture-conditioned generations
    if chat:
        prompt = f"""
        <<SYS>>
...
# The function to prompt the model for generating the culture-conditioned generations
                ...
                for gender in ["male", "female", ""]: # gender neutral baseline
                    ...
                    if model_path == "gpt-4":
                        ...
                        if chat:
                            ...
                            # if topic is sign on front door, generate more tokens (WILL CHECK LATER FOR TOXICITY)
...
def prompt_and_save(home_dir, model_name, model_path,
    ...
    nationalities = ["American", "Brazilian", "Chinese", "Filipino", "German", "Ghanaian", "Indian", "Japanese", "Mexican", "Russian"]
    ...
    # PATH_TO_SAVE
    if top_k != 50:
        PATH_TO_SAVE = f"{home_dir}/probable_data/categories_nationality_{num_samples}_{model_name}_prob={probably}_top_k={top_k}.json"
    else:
        if temp == 1.25:
            PATH_TO_SAVE = f"{home_dir}/probable_data/categories_nationality_{num_samples}_{model_name}_prob={probably}_[temp=1,25].json"
        elif temp == 0.75:
            PATH_TO_SAVE = f"{home_dir}/probable_data/categories_nationality_{num_samples}_{model_name}_prob={probably}_[temp=0,75].json"
        else:
            PATH_TO_SAVE = f"{home_dir}/probable_data/categories_nationality_{num_samples}_{model_name}_prob={probably}.json"

    
    topic_nationality_dict = prompting_pipeline(nationalities, 
                                                PATH_TO_SAVE,
                                                ...
                                                top_k_=top_k,
                                                temp_ = temp)
    with open(PATH_TO_SAVE, "w") as w:
        json.dump(topic_nationality_dict, w, indent=4)

# Using LLAMA-3 for extracting keywords or symbols from the generations
                ...
                if nationality not in new_topic_nationality_dict[topic][role]:
                    ...
                    for gender, generations in nationality_dict.items():
                        ...
                        for generation in generations:
                            ...
                            "Keyword Extraction from the Response using LLAMA-3"
...
def extract_keywords_from_long_value(model, topic, pronoun, value):
    ...
    # System prompt for this process
    sys_prompt = f"""
        Instructions:
...
if __name__ == "__main__":
    ...
    """
    RUN THE PROMPT ON THE LLM THROUGH THE PROMPTING PIPELINE for the ablation experiments
    """
    if args.prompt:
        """
        Ablating on top-k values
        """
        print("Generating top k = 20")
        prompt_and_save(args.home_dir, args.model_name, model_path, num_samples=args.num_samples, topic_list=args.topic_list, replace=args.overwrite, probably=args.probably, top_k = 20)
        print ("Generating top k = 80")

        """
        Ablating on temperature values
        """
        prompt_and_save(args.home_dir, args.model_name, model_path, num_samples=args.num_samples, topic_list=args.topic_list, replace=args.overwrite, probably=args.probably, top_k = 80)
        print("Generating temp = 0.75")
        prompt_and_save(args.home_dir, args.model_name, model_path, num_samples=args.num_samples, topic_list=args.topic_list, replace=args.overwrite, probably=args.probably, temp = 0.75)
        print("Generating temp = 1.25")
        prompt_and_save(args.home_dir, args.model_name, model_path, num_samples=args.num_samples, topic_list=args.topic_list, replace=args.overwrite, probably=args.probably, temp = 1.25)
    
    """
    SHORTENING THE ANSWERS BY STOPPING AT THE FIRST PERIOD for the ablation experiments
    """
    if args.shorten:
        print("Shortening top k = 20")
        posthoc_shorten_answer(f"{args.home_dir}/probable_data/categories_nationality_{args.num_samples}_{args.model_name}_prob={args.probably}_top_k=20.json", args.topic_list)
        print("Shortening top k = 80")
        posthoc_shorten_answer(f"{args.home_dir}/probable_data/categories_nationality_{args.num_samples}_{args.model_name}_prob={args.probably}_top_k=80.json", args.topic_list)
        print("Shortening temp = 0.75")
        posthoc_shorten_answer(f"{args.home_dir}/probable_data/categories_nationality_{args.num_samples}_{args.model_name}_prob={args.probably}_[temp=0,75].json", args.topic_list)
        print("Shortening temp = 1.25")
        posthoc_shorten_answer(f"{args.home_dir}/probable_data/categories_nationality_{args.num_samples}_{args.model_name}_prob={args.probably}_[temp=1,25].json", args.topic_list)